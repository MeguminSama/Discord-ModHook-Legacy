		...
		_Out_ LPPROCESS_INFORMATION lpProcessInformation)
{
	...
	std::filesystem::path pInjectDllPath = "ModHookInjection.dll";
	auto szInjectDllFullPath = std::filesystem::absolute(pInjectDllPath);

	LPCSTR DllsToInject[1] = {szInjectDllFullPath.string().data()};

	BOOL success = DetourCreateProcessWithDllsW(lpApplicationName, lpCommandLine, lpProcessAttributes, lpThreadAttributes, bInheritHandles, dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation, 1, DllsToInject, o_CreateProcessW);
